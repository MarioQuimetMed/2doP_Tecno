...
const form = useForm({
    pais: props.destino.pais,
    ciudad: props.destino.ciudad,
    nombre_lugar: props.destino.nombre_lugar,
    descripcion: props.destino.descripcion || "",
});

const submit = () => {
    form.put(resolveUrl("destinos/" + props.destino.id));
};
...
<template>
    <Head :title="`Editar ${destino.nombre_lugar}`" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center">
                <PencilSquareIcon class="h-6 w-6 mr-2 text-indigo-600" />
                <h2
