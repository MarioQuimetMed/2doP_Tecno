                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                            <div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-4">
                                <div class="flex items-center">
                                    <GlobeAltIcon class="h-8 w-8 text-indigo-600 dark:text-indigo-400" />
                                    <div class="ml-3">
                                        <p class="text-sm font-medium text-indigo-600 dark:text-indigo-400">Total Destinos</p>
                                        <p class="text-2xl font-bold text-indigo-900 dark:text-indigo-100">{{ destinos.total }}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
                                <div class="flex items-center">
                                    <MapPinIcon class="h-8 w-8 text-green-600 dark:text-green-400" />
                                    <div class="ml-3">
                                        <p class="text-sm font-medium text-green-600 dark:text-green-400">Países</p>
                                        <p class="text-2xl font-bold text-green-900 dark:text-green-100">{{ paises.length }}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
                                <div class="flex items-center">
                                    <EyeIcon class="h-8 w-8 text-purple-600 dark:text-purple-400" />
                                    <div class="ml-3">
                                        <p class="text-sm font-medium text-purple-600 dark:text-purple-400">En esta página</p>
                                        <p class="text-2xl font-bold text-purple-900 dark:text-purple-100">{{ destinos.data.length }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span
                                                ...
                                                :class="destino.planes_viaje_count > 0 
                                                    ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' 
                                                    : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400'"
                                            >
                                                {{ destino.planes_viaje_count }} {{ destino.planes_viaje_count === 1 ? 'plan' : 'planes' }}
                                            </span>
                                        </td>
                                        <td
                                        ...
                                        >
                                            <div class="flex justify-end space-x-2">
                                                <Link
                                                    :href="route('destinos.show', destino.id)"
                                                ...
                                                <button
                                                    @click="deleteDestino(destino.id, destino.nombre_lugar)"
                                                    class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300"
                                                    :class="{
                                                        'opacity-50 cursor-not-allowed': destino.planes_viaje_count > 0
                                                    }"
                                                    :disabled="destino.planes_viaje_count > 0"
                                                    :title="destino.planes_viaje_count > 0 ? 'No se puede eliminar: tiene planes asociados' : 'Eliminar'"
                                                >
                                    ...
                                    <tr v-if="destinos.data.length === 0">
                                        ...
                                        >
                                            ...
                                            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No hay destinos</h3>
                                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                                {{ search || selectedPais ? 'No se encontraron destinos con los filtros aplicados.' : 'Comienza creando un nuevo destino.' }}
                                            </p>
                                            <div class="mt-6" v-if="!search && !selectedPais">
                                                <Link
                                                    :href="route('destinos.create')"
                        ...
                        >
                            <div class="text-sm text-gray-500 dark:text-gray-400">
                                Mostrando <span class="font-medium">{{ destinos.from }}</span> a <span class="font-medium">{{ destinos.to }}</span> de <span class="font-medium">{{ destinos.total }}</span> destinos
                            </div>
                            <div class="flex space-x-2">
                                ...
                                <span class="inline-flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                                    Página {{ destinos.current_page }} de {{ destinos.last_page }}
                                </span>
                                <button
