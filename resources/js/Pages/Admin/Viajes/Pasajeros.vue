...
const totalIngresos = props.ventas.reduce((sum, v) => sum + parseFloat(v.monto_total), 0);
const pagosCompletados = props.ventas.filter(v => v.estado_pago === 'COMPLETADO').length;
</script>
                    ...
                    <div class="p-6">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <div class="flex items-center mt-1 text-gray-500 dark:text-gray-400">
                                    <MapPinIcon class="h-4 w-4 mr-1" />
                                    {{ viaje.plan_viaje?.destino?.nombre_lugar }}, {{ viaje.plan_viaje?.destino?.ciudad }}
                                </div>
                                <div class="flex items-center mt-1 text-gray-500 dark:text-gray-400">
                                    <CalendarDaysIcon class="h-4 w-4 mr-1" />
                                    {{ formatDate(viaje.fecha_salida) }} - {{ formatDate(viaje.fecha_retorno) }}
                                </div>
                            </div>
                            <div class="mt-4 md:mt-0 flex items-center space-x-4">
                                <div class="text-center">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Pasajeros</p>
                                </div>
                                <div class="text-center">
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">
                                    {{ pagosCompletados }} / {{ totalPasajeros }}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                >
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            :class="[
                                                ...
                                                getTipoPagoClasses(venta.tipo_pago)
                                            ]"
                                        >
                                            {{ venta.tipo_pago }}
                                        </span>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            :class="[
                                                'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium',
                                                getEstadoPagoClasses(venta.estado_pago)
                                            ]"
                                        >
                                            {{ venta.estado_pago }}
                                        </span>
                                ...
                                <tr v-if="ventas.length === 0">
                                    <td colspan="7" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                        <UsersIcon class="h-12 w-12 mx-auto mb-4 text-gray-300 dark:text-gray-600" />
                                        <p class="text-lg font-medium">No hay pasajeros registrados</p>
                                        <p class="text-sm mt-1">
                                            Aún no se han registrado ventas para este viaje
                                        </p>
                                    </td>
                                </tr>
                            </tbody>
