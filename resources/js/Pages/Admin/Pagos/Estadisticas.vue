...
const applyFilters = () => {
    router.get(route('pagos.estadisticas'), {
...
const totalMetodos = computed(() => {
    ...
    return props.pagosPorMetodo.reduce((sum, m) => sum + parseFloat(m.total), 0);
});
</script>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="p-3 bg-green-100 dark:bg-green-900/30 rounded-full">
                                <CurrencyDollarIcon class="h-10 w-10 text-green-600 dark:text-green-400" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Cantidad de Pagos</p>
                                <p class="text-3xl font-bold text-gray-900 dark:text-gray-100">
                                    {{ totales.cantidad_pagos }}
                                </p>
                            </div>
                            <div class="p-3 bg-blue-100 dark:bg-blue-900/30 rounded-full">
                                <BanknotesIcon class="h-10 w-10 text-blue-600 dark:text-blue-400" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Promedio por Pago</p>
                                <p class="text-3xl font-bold text-gray-900 dark:text-gray-100">
                                    {{ formatCurrency(totales.promedio_pago) }}
                                </p>
                            </div>
                            <div class="p-3 bg-purple-100 dark:bg-purple-900/30 rounded-full">
                                <ChartBarIcon class="h-10 w-10 text-purple-600 dark:text-purple-400" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div class="flex-1 mx-4">
                                    <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-6 relative">
                                        <div
                                            ...
                                            :style="{ width: maxPagoDia > 0 ? (parseFloat(dia.total) / maxPagoDia * 100) + '%' : '0%' }"
                                        >
                                            <span v-if="parseFloat(dia.total) / maxPagoDia > 0.3" class="text-xs text-white font-medium">
                                                {{ formatCurrency(dia.total) }}
                                            </span>
                                        </div>
                                        <span v-if="parseFloat(dia.total) / maxPagoDia <= 0.3" class="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-gray-600 dark:text-gray-400 font-medium">
                                            {{ formatCurrency(dia.total) }}
                                        </span>
                                    </div>
                                </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        <div v-if="pagosPorMetodo?.length" class="space-y-4">
                            ...
                            <div class="flex h-8 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700">
                                <div
                                    v-for="metodo in pagosPorMetodo"
                                    :key="metodo.metodo"
                                    :class="getMetodoBgClass(metodo.metodo)"
                                    :style="{ width: totalMetodos > 0 ? (parseFloat(metodo.total) / totalMetodos * 100) + '%' : '0%' }"
                                    class="transition-all duration-500"
                                    :title="`${metodo.label}: ${formatCurrency(metodo.total)}`"
                                ></div>
                            </div>
                            ...
                            <div class="space-y-3 mt-6">
                                ...
                                >
                                    <div class="flex items-center">
                                        <div :class="['w-4 h-4 rounded mr-3', getMetodoBgClass(metodo.metodo)]"></div>
                                        <component :is="getMetodoIcon(metodo.metodo)" class="h-5 w-5 mr-2 text-gray-500" />
                                        <span class="font-medium text-gray-900 dark:text-gray-100">{{ metodo.label }}</span>
                                    </div>
                                    <div class="text-right">
                                        <p class="font-bold text-gray-900 dark:text-gray-100">{{ formatCurrency(metodo.total) }}</p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ metodo.cantidad }} pagos</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-else class="text-center py-8 text-gray-500 dark:text-gray-400">
                            <CreditCardIcon class="h-12 w-12 mx-auto mb-2 text-gray-300" />
                            <p>No hay datos para el período seleccionado</p>
                        </div>
                    </div>
                </div>
