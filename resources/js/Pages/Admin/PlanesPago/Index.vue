...
const calcularProgreso = (plan) => {
    const pagadas = plan.cuotas.filter((c) => c.estado_cuota === "PAGADO").length;
...
const tieneCuotasVencidas = (plan) => {
    return plan.cuotas.some(
        (c) =>
            ...
            (c.estado_cuota === "PENDIENTE" && new Date(c.fecha_vencimiento) < new Date())
    );
};
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <Link
                    :href="route('planes-pago.dashboard')"
                        ...
                        <div class="flex items-center">
                            <div class="flex-shrink-0">
                                <div class="p-3 bg-purple-100 dark:bg-purple-900 rounded-full">
                                    <CreditCardIcon class="h-6 w-6 text-purple-600 dark:text-purple-400" />
                                </div>
                            </div>
                            <div class="ml-4">
                    ...
                    <div class="flex items-center">
                        ...
                        <div>
                            <p class="text-sm font-medium text-red-800 dark:text-red-200">
                                ¡Atención! Tienes {{ stats.cuotas_vencidas }} cuota(s) vencida(s) por un total de
                                {{ formatMoney(stats.monto_vencido) }}
                            </p>
                            <button
                                @click="conVencidas = true; applyFilters()"
                    ...
                    <div class="p-4">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button
                                    v-if="search || selectedEstado || selectedCuotas || conVencidas"
                                    @click="clearFilters"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div>
                                    ...
                                    >
                                        ...
                                        <option v-for="estado in estadosPago" :key="estado.value" :value="estado.value">
                                            {{ estado.label }}
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <option v-for="opcion in opcionesCuotas" :key="opcion.value" :value="opcion.value">
                                            {{ opcion.label }}
                                        </option>
                                    </select>
                                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                <tr
                                    ...
                                    :class="{ 'bg-red-50 dark:bg-red-900/10': tieneCuotasVencidas(plan) }"
                                >
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <div class="h-10 w-10 rounded-full bg-purple-100 dark:bg-purple-900 flex items-center justify-center">
                                                    <UserIcon class="h-5 w-5 text-purple-600 dark:text-purple-400" />
                                                </div>
                                            </div>
                                            <div class="ml-4">
                                                ...
                                                <div class="text-sm text-gray-500 dark:text-gray-400">
                                                    {{ plan.venta.viaje?.plan_viaje?.destino?.nombre || 'Sin destino' }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                            {{ plan.cuotas.filter(c => c.estado_cuota === 'PAGADO').length }}/{{ plan.cantidad_cuotas }} pagadas
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">
                                            {{ formatMoney(plan.venta.monto_total) }}
                                        </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            :class="getEstadoColor(plan.venta.estado_pago)"
                                            class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full"
                                        >
                                            {{ plan.venta.estado_pago }}
                                        </span>
                                        <div v-if="tieneCuotasVencidas(plan)" class="mt-1">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link
                                            :href="route('planes-pago.show', plan.id)"
                                ...
                                <tr v-if="planesPago.data.length === 0">
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        ...
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            Los planes de pago se crean automáticamente al realizar una venta a crédito.
                                        </p>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    >
                        <div class="flex items-center justify-between">
                            <div class="text-sm text-gray-700 dark:text-gray-300">
                                Mostrando {{ planesPago.from }} a {{ planesPago.to }} de
                                {{ planesPago.total }} resultados
                            </div>
                            <nav class="flex items-center space-x-1">
                                <template v-for="(link, index) in planesPago.links" :key="index">
                                    <Link
                                        v-if="link.url"
                                        :href="link.url"
